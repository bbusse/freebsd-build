#!/usr/bin/env bash
#
# Boots the built NanoBSD disk image under QEMU and verifies it reaches
# a console login prompt and that we can log in as root.
#
...
#
# NanoBSD images boot via legacy BIOS (boot0sio -> boot2 -> loader), not
# UEFI, so no OVMF firmware is needed here - qemu's default SeaBIOS is
# enough. The disk is MBR with a nested BSD label (see image-builder's
# nanobsd legacy.sh create_code_slice), attached via AHCI since that's
# compiled into GENERIC statically (unlike virtio-blk, which isn't
# guaranteed to be present).
#
# The root password and serial console are set post-build by
# image-builder's freebsd_customize_image() (mounts the image directly
# and runs pw -R + edits ttys/loader.conf) since NanoBSD's own
# NANO_CUSTOMIZE hooks run before the image exists.

DISK_IMAGE="${DISK_IMAGE:-nanobsd-amd64-full.img}"
